---
title: "The thousand ways to say everything is chaos"
lang: en
tags: [articulos, expresiones, vulgar]
translationKey: "Artículos/Las miles de maneras de decir que hay un caos"
date: "2026-07-27T03:44:39Z"
lastmod: "2026-07-27T03:44:39Z"
dateSource: "git-history"
---

> [!WARNING] This page is incomplete. #wip

Puerto Rican Spanish has countless ways to describe complete chaos in a place or situation. This page attempts to answer the most important question of our century:

> *¿Cuál crical es el peor crical?* — Which kind of chaos is the worst chaos?

## A rough hierarchy of intensity

```mermaid
flowchart TD
  A["Chaos / disorder<br/>(in a place or situation)"]
  A --> P["Words<br/>(nouns and labels)"]
  A --> D["Sayings<br/>(full phrases)"]
  P --> P1["Mild<br/>(neutral or not very vulgar)"]
  P --> P2["Moderate<br/>(colloquial)"]
  P --> P4["Extreme<br/>(very vulgar)"]
  D --> D2["Moderate<br/>(common phrase)"]
  D --> D3["Strong<br/>(a major mess broke out)"]
  P1 --> desorden["Un desorden<br/>(formal; neutral)"]
  P1 --> reguero["Reguero<br/>(mess; neutral)"]
  P2 --> revolu["Revolú<br/>(commotion; common)"]
  P2 --> peo["Un peo<br/>(mildly vulgar problem)"]
  P4 --> crical["Crical<br/>(vulgar; total chaos)"]
  P4 --> carajal["Carajal<br/>(very vulgar; total chaos)"]
  P4 --> despingue["Despingue<br/>(very vulgar; ugly mess)"]
  P4 --> jodienda["Una jodienda<br/>(very vulgar; problem/chaos)"]
  P4 --> descone["Descoñe<br/>(very vulgar; total chaos)"]
  P4 --> descojon["Un descojón<br/>(very vulgar; total chaos)"]
  D2 --> pastores["Se formó la de los pastores<br/>(a major commotion)"]
  D3 --> madre["Se formó la madre de los tomates<br/>(an enormous mess)"]
```

## Which word should you use?

- **Formal or inoffensive:** *un desorden* or *un reguero*.
- **Colloquial but not very vulgar:** [[en/Palabras/Revolú|*revolú*]].
- **A mild vulgar edge:** [[en/Palabras/Peo|*un peo*]].
- **Strong and vulgar:** [[en/Palabras/Crical|*crical*]], [[en/Palabras/Carajal|*carajal*]], [[en/Palabras/Despingue|*despingue*]], or [[en/Palabras/Jodienda|*una jodienda*]].
- **Extreme:** [[en/Palabras/Descoñe|*descoñe*]] or [[en/Palabras/Descojón|*un descojón*]].
- **As a full saying:** [[en/Dichos/La de los pastores|*se formó la de los pastores*]] or [[en/Dichos/La madre de los tomates|*se formó la madre de los tomates*]].

> [!NOTE] “Severity” combines intensity with how vulgar the expression sounds in casual conversation. Real usage varies by speaker and setting.

## Poorly named businesses

The only people in the Puerto Rican archipelago who can use *crical* with no irony or vulgarity at all: billiards players.

This is a perfect case study for why cultural review matters:

![crical billiards google search](/Art%C3%ADculos/_attachments/crical-billiards-google-search.png)
![crical billiards reddit](/Art%C3%ADculos/_attachments/crical-billiards-reddit.png)

If there had been one Boricua—just one—on that blessed team, this would not have happened. Now they have a… *crical*. Ja.

On the other hand…

![crical domain for sale](/Art%C3%ADculos/_attachments/crical-domain-for-sale.png)
